...
import { saveVitaRoutine } from "@/lib/firestore";
...
// ── Save Vita Routine Section ──

function SaveVitaRoutineSection({ answers, routine }: { answers: VitaAnswers; routine: VitaRoutine }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    setError(null);
    try {
      await saveVitaRoutine(user.uid, { answers, routine });
      setSaved(true);
    } catch {
  ...
  return (
    <Card className="border-primary/20" data-testid="card-save-vita-routine">
        ...
        {!user ? (
          <div>
            <p className="text-sm text-muted-foreground mb-4 leading-relaxed">
              Create a free account to save your personalized vitamin routine and access it anytime.
            </p>
            <Button
              variant="outline"
              size="sm"
              ...
              className="gap-2"
              data-testid="button-vita-sign-in-google"
            >
          ...
          <div className="text-center py-2" data-testid="vita-save-success">
          ...
          <div>
            <p className="text-sm text-muted-foreground mb-4 leading-relaxed">
              Signed in as <span className="font-medium text-foreground">{user.displayName ?? user.email}</span>. Save this routine to revisit it later.
            </p>
            <div className="flex gap-2 flex-wrap">
              <Button size="sm" onClick={handleSave} disabled={saving} className="gap-2" data-testid="button-save-vita-routine">
                <Bookmark className="w-3.5 h-3.5" /> {saving ? "Saving\u2026" : "Save Routine"}
              </Button>
...
function ShareVitaButton({ answers, profileCode }: { answers: VitaAnswers; profileCode: string }) {
  const [status, setStatus] = useState<"idle" | "copied" | "shared" | "failed">("idle");

  const handleShare = useCallback(async () => {
    const result = await shareVitaRoutine(answers, profileCode);
    setStatus(result);
    if (result === "copied") setTimeout(() => setStatus("idle"), 2500);
  }, [answers, profileCode]);

  return (
    <Button variant="outline" onClick={handleShare} className="gap-2 rounded-full" data-testid="button-share-vita-results">
      {status === "copied" ? (
        <><Check className="w-4 h-4" /> Link Copied</>
      ) : status === "shared" ? (
        <><Check className="w-4 h-4" /> Shared</>
      ) : (
        <><Share2 className="w-4 h-4" /> Share Routine</>
      )}
    </Button>
  );
}
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start p-4 rounded-xl bg-muted/40 border border-card-border">
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Core nutrients your profile points to as likely deficiencies or high-impact needs. Start here.
            </p>
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-1.5 mb-1">
              <Badge variant="outline" className="text-[10px] px-1.5 py-0 bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-950 dark:text-teal-300">Recommended</Badge>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Supplements with strong evidence for your goals. Add when you're ready — no need to take everything at once.
            </p>
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Nice-to-haves that may help with specific concerns. Consider only if budget and interest align.
            </p>
          </div>
        </div>

        {/* Save Routine — hide on shared views */}
        {!isSharedView && <SaveVitaRoutineSection answers={answers} routine={routine} />}
        ...
        <div className="flex flex-wrap gap-3 items-center justify-center">
          ...
          <ShareVitaButton answers={answers} profileCode={routine.profile.profileCode} />
          <Button onClick={() => { window.location.hash = "#/glow"; }} className="gap-2 rounded-full">
            Build Skincare Routine <ArrowRight className="w-4 h-4" />
          </Button>
        </div>
      </div>
    </div>
...
export default function VitaHome() {
  ...
  const [lastAnswers, setLastAnswers] = useState<VitaAnswers>({});
  ...
  return (
    <div>
      ...
      {appState === "results" && routine && (
        <VitaResults routine={routine} answers={lastAnswers} onRetake={handleRetake} />
      )}
    </div>
  );
}
